 ...
 * ExternalNameLinkTag
 */
public class ExternalLabelLinkTag extends Tag {

    public ExternalLabelLinkTag(String regex) {
        super(regex);
    }

    /**
     * Replaces "[http://localhost localhost]" with "<a href="http://localhost">localhost</a>"
     * @param content
     * @return
     */
    public StringBuffer parseContent(StringBuffer content, Parser parser) {
        ...
        while (m.find()) {
            String replace = new Link(getRequestContext()).setPath(m.group(1))
            		.setStyleClass(MediawikiParser.getLinkCssClass(m.group(2)))
            		.setEscapeTitle(m.group(3)).getString();
