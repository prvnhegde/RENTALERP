 ...
 * NoSpaceMatchingTags
 */
public class NoSpaceMatchingTags extends Tag {

    public NoSpaceMatchingTags(String regex) {
        super(regex);
    }

    public StringBuffer parseContent(StringBuffer content, Parser parser) {
        StringBuffer sb = new StringBuffer();
        Matcher m = Pattern.compile(getRegex()).matcher(content);
        while (m.find()) {
            // No space between the text and the indicators.
            if (!m.group(1).startsWith(" ") && !m.group(1).endsWith(" ")) {
                String replace = getOpenTag() + m.group(1) + getCloseTag();
