 ...
 * InternalLinkTag
 */
public class InternalLinkTag extends Tag {

    public InternalLinkTag(RequestContext requestContext, String regex) {
        super(requestContext, regex);
    }
    ...
    public StringBuffer parseContent(StringBuffer content, Parser parser) {
        ...
        while (m.find()) {
            String replace = new Link(getRequestContext()).setAppPath(MediawikiParser.WIKI_URL
                    + KbUtils.formatWikiNamespace(AttributeFieldIds.KB_ARTICLE_SYNTAX_MEDIAWIKI, m.group(1))).setEscapeTitle(m.group(1)).getString();
