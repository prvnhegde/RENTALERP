...
public class ExternalLinkTag extends Tag {
    ...
    /**
     * Replaces "http://localhost" with "<a href="http://localhost">http://localhost</a>"
     * @param content
     * @return
     */
    public StringBuffer parseContent(StringBuffer content, Parser parser) {
        ...
        while (m.find()) {
            if (!m.group(1).equals("[") && !m.group(4).startsWith("]")) {
                String replace = m.group(1) + new Link(getRequestContext()).setPath(m.group(2))
                		.setStyleClass(MediawikiParser.getLinkCssClass(m.group(3)))
                		.setEscapeTitle(m.group(2)).getString() + m.group(4);
