...
public class InternalLabelLinkTag extends Tag {
    ...
    public InternalLabelLinkTag(RequestContext requestContext, String regex) {
    ...
    public StringBuffer parseContent(StringBuffer content, Parser parser) {
        ...
        while (m.find()) {
            String replace = new Link(getRequestContext()).setAppPath(MediawikiParser.WIKI_URL
                    + KbUtils.formatWikiNamespace(AttributeFieldIds.KB_ARTICLE_SYNTAX_MEDIAWIKI, m.group(1))).setEscapeTitle(m.group(2)).getString();
