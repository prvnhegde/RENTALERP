...
--
DROP VIEW if exists admin_perm_page_map_view;
...
create or replace view asset_hardware_view as 
	select ah.hardware_id, ah.hardware_number, ah.hardware_name, ah.hardware_description, ah.hardware_type, ah.hardware_status, 
		ho.display_name as hardware_owner_display_name, ho.username as hardware_owner_username, ah.hardware_owner as hardware_owner_id, 
		ah.hardware_model_name, ah.hardware_model_number,
...
create or replace view attribute_view as 
	select a.attribute_id, a.object_type_id, so.object_key, a.attribute_key as attribute_name, a.attribute_type, a.attribute_option, a.attribute_convert_url,
		...
		a.input_mask, a.description, a.type_currency_symbol
...
create or replace view contract_view as 
	select c.contract_id, c.contract_name, c.contract_description, c.contract_owner as contract_owner_id, 
		co.display_name as contract_owner_display_name, co.username as contract_owner_username, 
		...
		c.contract_expiration_notified,
...
--
DROP VIEW if exists company_tag_map_view;
...
create or replace function sp_attribute_add(out o_attribute_id int, in p_object_type_id int, in p_attribute_name varchar(50),
    ...
    in p_attribute_url text, in p_default_attribute_field_id int, in p_attribute_group_id int, in p_input_mask varchar(50), in p_type_currency_symbol varchar(10), in p_is_required int)
RETURNS integer AS $$
begin
	select nextval('seq_attribute_id') into o_attribute_id;

	insert into attribute (attribute_id, object_type_id, attribute_key, description, attribute_url, attribute_type, attribute_option, attribute_convert_url, is_editable, default_attribute_field_id, attribute_group_id, input_mask, type_currency_symbol, is_required, is_custom_attr)
		values (o_attribute_id, p_object_type_id, p_attribute_name, p_description, p_attribute_url, p_attribute_type, p_attribute_option, p_attribute_convert_url, 1, p_default_attribute_field_id, p_attribute_group_id, p_input_mask, p_type_currency_symbol, p_is_required, 1);
end;
$$ LANGUAGE 'plpgsql';
...
create or replace function sp_attribute_update(in p_attribute_id int, in p_attribute_name varchar(50), in p_description varchar(255),
    ...
    in p_input_mask varchar(50), in p_type_currency_symbol varchar(10), in p_is_required int)
RETURNS void AS $$
begin
	update attribute set attribute_key = p_attribute_name,
		...
		type_currency_symbol = p_type_currency_symbol,
...
begin
	...
	FOR license_record IN select license_id from asset_software_licenses where software_id = p_software_id
	loop
...
RETURNS void AS $$
begin
	update contract set contract_expiration_notified = null 
		where contract_id = p_contract_id 
		and ((p_contract_expiration_date is null or to_timestamp(p_contract_expiration_date, 'YYYY-MM-DD HH24:MI:SS') > now())
				and (contract_expiration_date <> to_timestamp(p_contract_expiration_date, 'YYYY-MM-DD HH24:MI:SS')
						or (p_contract_owner is null or contract_owner <> p_contract_owner)));
...
--
create or replace function sp_kb_article_add(out o_article_id int, in p_article_name varchar(100), in p_article_text text, in p_article_syntax_type int, in p_article_wiki_namespace varchar(100), in p_category_id int, in p_event_type_id int, in p_creator int)
RETURNS int AS $$
	declare v_now timestamp(1);
begin
	select now() into v_now;

	insert into kb_article(article_id, article_name, article_text, article_syntax_type, article_wiki_namespace , category_id, creator, creation_date)
		values (nextval('seq_kb_article_id'), p_article_name, p_article_text, p_article_syntax_type, p_article_wiki_namespace, p_category_id, p_creator, v_now);
...
--
create or replace function sp_kb_article_update(in p_article_id int, in p_article_name varchar(100), in p_article_text text, in p_article_syntax_type int, in p_article_wiki_namespace varchar(100), in p_category_id int, in p_event_type_id int, in p_modifier int)
RETURNS void AS $$
...
begin
	...
	update kb_article set article_name=p_article_name, article_text=p_article_text, article_syntax_type=p_article_syntax_type, 
			article_wiki_namespace = p_article_wiki_namespace,
